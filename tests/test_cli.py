...
def test_run_baseline_pipeline_orders_steps(monkeypatch, tmp_path: Path) -> None:
    ...
    config = ProjectConfig(
        ...
        macro_indicators={
            "us_unemployment": SourceConfig(
                country="US",
                source="fred_unemployment_rate",
                raw_file=tmp_path / "unrate.csv",
            )
        },
    )
    ...
    monkeypatch.setattr(
        ...
        single_step("build_learned_model_findings"),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        single_step("build_macro_conditioned_representation_summary"),
    )
    monkeypatch.setattr(
        cli,
