...
def test_summarize_baselines_writes_csv_tables(tmp_path: Path) -> None:
    ...
    curve_state_probe_importance = pd.read_csv(output_paths[12])
    ae_classical_correlations = pd.read_csv(output_paths[13])
    benchmark_conclusions = pd.read_csv(output_paths[14])
